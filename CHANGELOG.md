...
## 0.5.7 - 2026-09-21 (render 의 실제 동작 범위를 드러냄)

### 확인 - `render` 는 국제판 CapCut 에서 동작하지 않는다

실제로 돌려보고 알아낸 사실이다. `render` 는 upstream `pycapcut` 의
`JianyingController` 에 위임하는데, 그 코드는 창을 이렇게 고른다.
...
국제판 CapCut 은 창 이름이 `CapCut` 이라 이 비교가 절대 참이 되지 않는다
(CapCut 9.4.0 에서 실측: `AutomationError: 剪映窗口未找到`). 게다가 매칭 이후에도
`导出` 같은 중국어 UI 라벨을 찾으므로, 창 이름만 맞춰도 동작하지 않는다.

그동안 README 는 `render` 를 제약 없이 소개했다. 해외 사용자가 이 명령을 시도하면
원인을 알 수 없는 중국어 에러만 본다.

- 실패 시 진짜 원인과 대안(`render-headless`)을 알려준다.
- 모듈 독스트링과 README(영/한)에 '중국판 전용'을 명시하고, 렌더 예시의 순서를
  `render-headless` 우선으로 바꿨다.
- `render` 자체는 그대로 둔다. 중국판 사용자에게는 유효한 경로다.
...
`text add --style` 같은 명령이 실측 3,937ms 였다. 폰트를 쓰지 않는 `track add` 는
371ms 다. 차이의 전부가 폰트 스캔이었다.

원인은 CapCut 캐시(`Cache/effect`) 를 `rglob("*")` 로 전수 순회한 것이다. 이 디렉토리는
폰트만이 아니라 다운로드한 모든 에셋이 들어가서 실측 26,353 개 항목이었고, 그걸 다 훑어
폰트는 5 개를 찾았다. 캐시 스캔에만 2.8 초가 들었다.

CapCut 캐시는 `<resource_id>/<hash>/<파일>` 구조라 폰트가 깊이 3 에 놓인다. 전수 순회
대신 깊이를 제한(1~4)해 찾는다. 나머지 세 경로(CapCut SystemFont / 사용자 Fonts /
Windows Fonts)는 얕아서 기존 방식 그대로다.

- 별칭 결과는 동일하다(387 개, 캐시에서 찾은 파일 5 개도 동일). 동작 변화 없음.
- `text add --style` 3,937ms -> 1,664ms.
- 부수 효과로 테스트 전체가 83 초 -> 64 초.

프로세스 간 디스크 캐시는 **일부러 넣지 않았다.** 남은 시간은 478 개 폰트 파일의 cmap
파싱이고 그건 실제로 하는 일이다. 캐시 무효화가 어긋나면 "폰트를 못 찾는" 회귀가
생기는데, 방금 폰트 해석 버그를 고친 직후에 그 리스크를 지는 것은 이득보다 손해가 크다.

### 견고성 - subprocess 3 곳에 타임아웃이 없었다

`asset cover-from-frame`(ffmpeg), `render-headless`(ffmpeg), `text auto-srt`(whisper)
가 상한 없이 실행되고 있었다. 입력이 손상됐거나 응답 없는 경로를 만나면 무한 대기한다.
이 CLI 는 AI 에이전트가 호출하는 것을 전제하므로, 멈춘 자식 프로세스 하나가 에이전트
세션 전체를 잡는다.

- 커버 추출 120 초, 렌더 3600 초, whisper 3600 초. 초과 시 무엇이 왜 멈췄는지 알린다.
- 이로써 subprocess 호출 6 곳 전부에 상한이 있다.

### 수정 - 의존성 상한이 너무 넓었다

`pycapcut>=0.0.3,<1.0.0` 이었다. 0.0.x 는 아직 불안정 버전대이고, 실제로 0.0.3 과
upstream git 사이에 `TextSegment(shadow=...)` 유무 차이가 있었다. `<0.1.0` 으로 좁혔다.
